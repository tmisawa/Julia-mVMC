...
- **MPI** — multi-rank sample-parallel execution with `NSplitSize = 1` is supported through MPI.jl-compatible launchers. `VMCParaOpt` MPI supports the direct SR solver (`NSRCG = 0`) and the standard SR-CG solver (`NSRCG = 1`). C's grouped MPI/QP split (`NSplitSize > 1`), additional CG modes (`NSRCG >= 2`), diagonal-scaling CG (`useDiagScale != 0`), and S-matrix rescaling (`RescaleSmat != 0`) are still rejected.
- **Not supported in this release**: BackFlow (`vmc_bf_*` entry points raise an error), full Lanczos (only step-0 is comparable), grouped MPI/QP splitting (`NSplitSize > 1`), `NSRCG >= 2`, `useDiagScale != 0`, and `RescaleSmat != 0`.
